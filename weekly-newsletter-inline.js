  ...
  function newsletterToolbarV820(latest,week,body){
    return '<div class="weeklyNewsletterV820"><div class="newsletterToolbarV820"><label>Newsletter Week<select id="newsletterWeekPickerV820">'+newsletterWeekOptionsV820(latest,week)+'</select></label><div class="hubFilterNote">Weekly Chronicle archive</div></div>'+body+'</div>';
  }
  ...
  function renderWeeklyNewsletterV820(){
    ...
    // Preserve Week 1 exactly as it was published last week.
    if(Number(rec.nflSeason)===2026&&week===1){
      content.innerHTML=newsletterToolbarV820(latest,week,'<div class="newsletterFrameWrapV820"><iframe class="newsletterFrameV820" title="Week 1 Newsletter" src="./chronicle.html?week=1"></iframe></div>');
      newsletterBindPickerV820();
      return;
    }

    // Week 2 onward is built directly from the active season/Sleeper feed.
